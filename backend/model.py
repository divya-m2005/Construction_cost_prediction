...
def encode_input(data, label_encoders: dict) -> np.ndarray:
    ...
    for col in categorical_cols:
        ...
        if col in label_encoders:
            ...
            mapping = {v: i for i, v in enumerate(label_encoders.get(col, [val]))}
...
def get_cost_breakdown(total_cost: float, data) -> dict:
    """Generate approximate cost breakdown by category."""
    # Approximate percentages based on industry standards
    percentages = {
...
def get_recommendations(data, predicted_cost: float) -> list:
    ...
    if data.quality_grade.value == "luxury":
        recommendations.append("Consider premium grade to reduce cost by ~15% with minimal quality impact.")

    if data.wall_material.value == "steel" and data.project_type.value == "residential":
...
def predict_cost(data, model, scaler, label_encoders) -> dict:
    """Run prediction pipeline and return structured output."""
    # If no model is loaded, use a heuristic fallback
    if model is None:
        base_cost_per_sqft = {
            "economy": 80, "standard": 130, "premium": 200, "luxury": 320
        }.get(data.quality_grade.value, 130)

        zone_multiplier = {
            "urban": 1.3, "suburban": 1.0, "rural": 0.8
        }.get(data.location_zone.value, 1.0)

        material_multiplier = {
            "brick": 1.0, "concrete": 1.05, "steel": 1.15,
            "wood": 0.9, "glass": 1.25
        }.get(data.wall_material.value, 1.0)
        ...
        cost_per_sqft = predicted_cost / data.area_sqft
        confidence = 0.91

    cost_per_sqft = predicted_cost / (data.area_sqft * data.num_floors)

    # 90% confidence interval (±12%)
